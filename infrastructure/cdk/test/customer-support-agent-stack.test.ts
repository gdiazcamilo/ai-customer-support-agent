...
beforeAll(() => {
	...
	const stack = new CustomerSupportAgentCdkStack(app, "TestStack", {
		environmentName: "dev",
		bedrockPromptParameter:
			"/ai-customer-support-agent/dev/bedrock-system-prompt",
	});
...
test("connects the jobs queue to the worker Lambda", () => {
	...
	template.hasResourceProperties("AWS::Lambda::EventSourceMapping", {
		EventSourceArn: {
			"Fn::GetAtt": [jobsQueueLogicalId, "Arn"],
		},
		...
		Environment: {
			Variables: Match.exact({
				...
				AGENTCORE_RUNTIME_ARN: {
					"Fn::GetAtt": [runtimeLogicalId, "AgentRuntimeArn"],
				},
			}),
		},
	});
...
test("configures AgentCore Runtime only with its service dependencies", () => {
	...
	template.hasResourceProperties("AWS::BedrockAgentCore::Runtime", {
		EnvironmentVariables: Match.exact({
			...
			KNOWLEDGE_BASE_ID: {
				"Fn::GetAtt": [knowledgeBaseLogicalId, "KnowledgeBaseId"],
			},
			AGENTCORE_MEMORY_ID: {
