...
export class AgentCore extends Construct {
	...
	constructor(scope: Construct, id: string, props: AgentCoreProps) {
		...
		this.memory = new agentcore.Memory(this, "Memory", {
			...
			description:
				"Short-term conversational memory for the customer support agent",
		...
		this.runtime = new agentcore.Runtime(this, "Runtime", {
			...
			environmentVariables: {
				...
				LOG_LEVEL: "INFO",
				BEDROCK_MODEL_ID: this.BEDROCK_MODEL_ID,
			...
			new iam.PolicyStatement({
				...
				resources: [
					cdk.Stack.of(this).formatArn({
						...
						resourceName: this.BEDROCK_MODEL_ID,
					}),
				],
			}),
		);
			...
			new iam.PolicyStatement({
				...
				resources: [
					cdk.Stack.of(this).formatArn({
						service: "ssm",
						resource: "parameter",
						resourceName: props.bedrockPromptParameter.replace(/^\//, ""),
					}),
				],
			}),
		);
